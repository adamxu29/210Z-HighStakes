...
void Eclipse::Autonomous_Paths::skills()
{
	...
	pros::delay(250);
	chassis.moveTo(0, 23, 900);

	// turn and intake ring 2
	r_pid.set_r_constants(2.5, 0, 15);
	r_pid.rotation_pid(26.5, 90, 0.8);

	chassis.setPose(0, 0, 0);
	chassis.moveTo(0, 60, 900);
	chassis.moveTo(0, 73.2, 900, 80); // make sure x is aligned with top ring
	pros::delay(500);
	...
	intake.move_voltage(0);
	driver.next_state();
	pros::delay(500); // shorten this delay if the lb moves into pos in time
	intake.move_voltage(12000);

	chassis.setPose(0, 0, 0);
	chassis.moveTo(0, 15, 900, 75);

	// intake ring 4 for lb

	chassis.setPose(0, 0, 0);
	chassis.turnTo(-12, -50, 900, true);
	chassis.moveTo(-12, -50, 1000); // move in front of ring
	intake.move_voltage(0);
	driver.next_state();

	r_pid.set_r_constants(2.5, 0, 15);
	r_pid.rotation_pid(90, 90, 0.8); // face wallstake
	intake.move_voltage(12000);

	chassis.setPose(0, 0, 0);
	chassis.moveTo(0, 28, 1500, 50); // intake ring
	pros::delay(300);				 // intake delay
	...
	r_pid.set_r_constants(2.5, 0, 15);
	r_pid.rotation_pid(180, 90, 0.8);

	intake.move_voltage(12000);

	chassis.setPose(0, 0, 0);
	chassis.moveTo(0, 1, 900, 75);
}
